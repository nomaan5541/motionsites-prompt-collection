#!/usr/bin/env bash
# Downloads every preview image from motionsites.ai and sorts into folders.
set -u

BASE="F:/preview/motionsites-assets"
mkdir -p "$BASE/hero" "$BASE/footer" "$BASE/features" "$BASE/landing" \
         "$BASE/cta" "$BASE/logos" "$BASE/misc"
...
for entry in "${ITEMS[@]}"; do
  ...
  case "$name" in
    ...
    landing-*)     dir="landing" ;;
    cta-*)         dir="cta" ;;
    *logo*.png|*-logo.png) dir="logos" ;;
    *)             dir="misc" ;;
  esac
  out="$BASE/$dir/$name"
  if curl -fsSL --retry 3 --retry-delay 1 -A "Mozilla/5.0" -o "$out" "$url"; then
    ok=$((ok+1)); printf '  OK   [%-9s] %s\n' "$dir" "$name"
  else
    fail=$((fail+1)); printf '  FAIL [%-9s] %s  <- %s\n' "$dir" "$name" "$url"
  fi
done

echo "----------------------------------------"
echo "Downloaded: $ok   Failed: $fail   Total: ${#ITEMS[@]}"
